...
sf_locs <- st_as_sf(tbl_locs, coords = c("longitude","latitude")) %>% 
  ...
  dplyr::mutate(id = ifelse(deployid == unique(deployid)[1], 1, 2))

sf_lines <- sf_locs %>%
  st_geometry() %>% 
  st_cast("MULTIPOINT",ids = sf_locs$id) %>% 
...
tbl_locs %>% group_by(deployid) %>% do(head(.,1))
```
...
`date_hour` field within `tbl_locs` as the basis for our join.
...
was not transmitted back and so are not included in the source data. This will 
...
The last step we need to complete is to translate the simple feature geometry
for our point coordinates into an `x` and `y` column.

```{r}
coords <- sf_locs_activity$geometry %>% map(unlist)

coords_df <- list()

for (i in 1:length(coords)) {
  coords_df[[i]] <- data.frame(x = ifelse(!is.null(coords[[i]]),
                                               coords[[i]][,1],
                                               NA),
                               y = ifelse(!is.null(coords[[i]]),
                                               coords[[i]][,2],
                                               NA))
}

sf_locs_activity <- sf_locs_activity %>% 
  bind_cols(bind_rows(coords_df))
