...
## Adjust Path Around Land

The `crawl` package now includes a `fix_path()` function which will adjust any
predicted path around land features using a least-cost function. For this
function, you'll need a raster representation of your land (or other feature
to avoid). In this case, we are pulling from the nPacMaps R packages (available
on Github at https://github.com/jmlondon/nPacMaps) which includes a basemap
for the Bering Sea and Aleutian Islands.

Additional parameters of importance are an expansion amount (`poly_expand`)
which expands the study area beyond the range of our predicted track. We also
need to set the raster resolution (`res`). In this case, our area of interest
is relatively small and we need a lower resolution value (200 meters) in order 
to capture all of the land detail. Such a high resolution raster is NOT advised
for larger scale studies. The remainder of the code example below simply
creates the raster and sets up the transition layer needed to calculate the
least-cost path.

```{r}
bering_base <- nPacMaps::bering(fortify = FALSE, simplify = FALSE)
bering_base@polygons <- lapply(bering_base@polygons,  
...
r <- raster::raster(
  ext = raster::extend(raster::extent(matrix(st_bbox(sf_pred_lines), 
                                      ...
                                      poly_expand),
                       resolution = res,
                       crs = sp::CRS("+init=epsg:3571")
)
land <- raster::rasterize(bering_base,
                          r,
                          getCover = TRUE,
                          background = 0,
                          filename = tmpland)
...
The `crawl::fix_path` function will return a 2-column matrix of `mu.x` and 
`mu.y` values. This matrix is of the same length as the `crwPredict` object and
we need to replace the original coordinates with our _fixed_ values. To make
this easier within our functional approach, we create a wrapper function,
`.fix_path` that calls `crawl::fix_path` and then edits the original data. The
last step is to create a combined `sf` object that has each of our _fixed_ lines
and we can use this for any plotting.
...
tbl_locs_activity_fit <- tbl_locs_activity_fit %>% 
  dplyr::mutate(sf_points_fix = purrr::map(predict,
                                           .fix_path,
                                           res_raster = land,
                                           trans = trans))

tbl_locs_activity_fit <- tbl_locs_activity_fit %>%  
  dplyr::mutate(sf_lines_fix = purrr::map2(sf_points_fix,deployid,
                                           as.sf,
                                           epsg = 3571,
                                           type = "LINE",
                                           loctype = "p"))

sf_pred_lines_fix <- tbl_locs_activity_fit$sf_lines_fix %>% 
  lift(rbind)() %>% sf::st_set_crs(3571) 

```

(ref:leaflet-pred-lines-fix-fig-cap) An interactive map of predicted tracks from two harbor seals in the Aleutian Islands. The paths have been _fixed_ to re-route around land features.

```{r, leaflet-pred-lines-fix, out.width = '100%', fig.cap = '(ref:leaflet-pred-lines-fix-fig-cap)'}
n <- length(unique(sf_pred_lines_fix$deployid))
pal <- colorFactor(ggthemes::ptol_pal()(n), 
                   domain = sf_pred_lines_fix$deployid)
